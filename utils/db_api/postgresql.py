import abc
from typing import (Optional, Union, List, overload, Literal, final, Tuple, Any, Dict, Protocol)
...
class WithDefaultsValues(Protocol):
...
class BaseDB(abc.ABC):
    ...
    @overload
    async def select_entry(self, fetch_all: Literal[True], **kwargs: Any) -> Optional[List[asyncpg.Record]]:
        ...

    @overload
    async def select_entry(self, fetch_all: Literal[False], **kwargs: Any) -> Optional[asyncpg.Record]:
        ...
    ...
    async def delete_entry(self, **kwargs: Any) -> str:
        ...
        sql, parameters = self.format_arguments(sql, kwargs)
        return (await self.execute(sql, *parameters, execute=True)).replace("DELETE ", "")

    @logger.catch
    async def update_entry(self, update_values: Dict[str, Any], **kwargs: Any) -> Optional[List[asyncpg.Record]]:
        update_params = "".join(
